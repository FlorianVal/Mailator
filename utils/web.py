...
class webScrap(object):
    ...
    def __getConfig(self):
        with open('config/config.yaml') as yaml_data_file:
            self.config = yaml.load(yaml_data_file, Loader=yaml.FullLoader).get("Web")
    ...
    def submitForms(self, email, first_name=None, last_name=None):
        ...
        logging.info(f"Submit form to {url} with : {email}, first_name: {first_name}, last_name: {last_name}")
    ...
    def __checkFormsDetails(self, forms):
        for i, form in enumerate(forms):
            ...
            logging.info(f"Hidden inputs : {len([input for input in form_details['inputs'] if input['type'] == 'hidden'])}")
            
            inputs = [input for input in form_details["inputs"] if input["type"] != 'hidden' and input["type"] != "submit"]
            ...
            if len(set(name_in_conf) - set(inputs_name)) != 0:
                logging.warning(f"Name in Form conf not found in inputs : {set(name_in_conf) - set(inputs_name)}")
            if len(set(inputs_name) - set(name_in_conf)) != 0:
                logging.warning(f"Name in inputs not found in conf : {set(inputs_name) - set(name_in_conf)}")

    @staticmethod
    def getNameFromMail(email):
        try:
            a, b = email.split("@")[0].split("_")[0], email.split("@")[0].split("_")[1]
        except IndexError:
            a, b = email.split("@")[0][:len(email.split("@")[0])//2], email.split("@")[0][len(email.split("@")[0])//2:]
        return a, b