...
class MailHandler(object):
    ...
    def GetRandomMailAddress(self):
        ...
        self.__checkDomains()
        mail_address = str(
            random_name + random.choice(self.domains["domains_list"])).lower()

        if hasattr(self, "managed_address"):
            self.managed_address.append(
                {"mail_address": mail_address, "created": datetime.now()})
        else:
            self.managed_address = [
                {"mail_address": mail_address, "created": datetime.now()}]
        ...
        logging.info(
            f"Managed address : {list(map(lambda x:x.get('mail_address'), self.managed_address))}")
    ...
    def __GetMailsFromInbox(self, mail_address):
        ...
        url = self.config.get(
            "ApiUrl") + f"/request/mail/id/{hashlib.md5(mail_address.encode()).hexdigest()}/"
        ...
        try:
            ...
            self.managed_address[index].update(
                {"mails": ast.literal_eval(response.text)})
        except ValueError:
            self.managed_address.append(
                {"mail_address": mail_address, "mails": ast.literal_eval(response.text)})

        logging.info(
            f'Get mails : {url} from : {mail_address} Response : {response.status_code}, number of message : {len(ast.literal_eval(response.text)) if type(ast.literal_eval(response.text)) is list else 0}')
    ...
    def __purgeManagedAddress(self):
        for index, address in enumerate(self.managed_address):
            ...
            if "created" not in address:
                logging.error(
                    f"{address.get('mail_address')} has no creation time")
